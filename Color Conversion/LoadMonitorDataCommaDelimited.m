...
function ColorInfo = Load_Monitor_Data_Comma_Delimited(FileName)
try
    ...
    %ColorInfo = Initialize_Color_Info_Structure();
    fid = fopen(FileName);
    if fid > 0 %make sure file exists
        ...
        junk = textscan(fid, '%s', 2);%#ok<NASGU>
        input = textscan(fid, '%u',1);
        ...
        while (feof(fid) ~= 1)
            ...
            junk = textscan(fid, '%c', 1);%#ok<NASGU>
            input = textscan(fid, '%u',2); IntIntensity = input{1,1}+1; clear input;
            junk = textscan(fid, '%c', 1); %#ok<NASGU>
            input = textscan(fid, '%u',2); IntReading = input{1,1}+1; clear input;
        ...
        ColorInfo.ColorMatchingFunctionsX = ColorMatch.X;
        ColorInfo.ColorMatchingFunctionsY = ColorMatch.Y;
        clear ColorMatch
